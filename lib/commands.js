...
var commands = {
  ...
  'Inspector Depth': {
    ...
    action: function(depth){
       ...
       this.clearInput();
       this.timedMessage('depth is ' + this.settings.depth, style.prompt.toggleOn);
     }
   },
  ...
  'Inject REPL': {
    ...
    action: function(){
      this.context._ = this.context.repl = this;
...
function toggle(prop){
  return function(){
    ...
    if (this.settings[prop]) {
      this.timedMessage('++' + prop, style.prompt.toggleOn);
    } else {
      this.timedMessage('--' + prop, style.prompt.toggleOff);
    }
  }
}
