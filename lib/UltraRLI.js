...
function UltraRLI(stream, completer){
  ...
  if (!this.enabled) {
    ...
    mounts = require('../settings/mounts')(output.getWindowSize());
...
UltraRLI.prototype = Object.create(RLI.prototype, {
  ...
  erase: __(function erase(mount){
    if (mount.width === 0) return;
    var x;
    if (mount.align === 'right') {
      x = mount.x - mount.width;
    ...
    this.cursor = this.line.alength;
    this.output.cursorTo(x, mount.y);
    this.output.write(' '.repeat(mount.width));
    mount.width = 0;
  }),

  writeMount: __(function writeMount(name, message){
    var mount = mounts[name], x;
    this.cursor = this.line.alength;
    mount.width = message.alength;
    if (mount.align === 'right') {
      x = mount.x - mount.width;
    } else if (mount.align === 'center') {
      x = mount.x - (mount.width / 2 | 0);
    } else {
      x = mount.x;
    }
    this.output.cursorTo(x, mount.y);
  ...
  timedWrite: __(function timedPrompt(name, message, time){
    var mount = mounts[name];
    this.writeMount(name, message);
    clearTimeout(mount.timer);
    mount.timer = setTimeout(function(){
      this.erase(mount); this.toCursor();
    }.bind(this), time || 5000)
  }),
