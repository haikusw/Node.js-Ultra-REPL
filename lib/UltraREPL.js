...
var maxLength = require('./utilities').maxLength;
//var ui = require('./ui');
...
UltraREPL.prototype = Object.create(REPLServer.prototype, {
  ...
  showHelp: __(function showHelp(){
    var nameMax = maxLength(this.help, 'name') + 2;
    var actMax = maxLength(this.help, 'activation') + 2;
    var lines = this.help.length;
    var descWidth = this.rli.output.getWindowSize()[0] - nameMax - actMax - 8;
    this.print([this.help.map(function(command){

      if (command.type === 'keywords' && command.activation !== '') {
        command.activation = chunk(command.activation, ', ', [descWidth - 10, descWidth]);
        lines += command.activation.length;
        command.activation = indent(command.activation, nameMax + actMax + 2);
        command.help += '\n' + colors(command.activation, 'bblack');
        command.activation = '';
      }

      return margins(nameMax, colors(command.name, 'cyan')) +
             margins(actMax, colors(command.activation, this.typeColors[command.type])) +
             command.help;
    }.bind(this)).join('\n')]);
    this.clear(-lines);
    this.displayPrompt();
